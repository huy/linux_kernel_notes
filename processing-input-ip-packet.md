...
Due to presence of netfilter firewall component, the ip_rcv however just perform sanity check (e.g. ip version, checksum, drop frame to other host), then invoke netfilter hook if passed, ip_rcv_finish will perform the main of work (decide of local delivery vs. forward, parse ip options).
